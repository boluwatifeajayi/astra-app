...
const StudentChat = () => {
  ...
  const fetchTutorData = async () => {
    try {
      ...
      const response = await axios.get(`${server}/students/get/${studentId}`, {
  ...
  const fetchChatData = async () => {
    ...
    try {
      const token = await AsyncStorage.getItem('tutorToken');
      const response = await axios.get(`${server}/messages/conversation/${studentId}/Student`, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      await axios.post(
        ...
        { recipientId: studentId, content: message },
        { headers: { Authorization: `Bearer ${token}` } }
      );
    } catch (error) {
        ...
        <View className="flex-1">
          ...
          >
            {chat.map((item:any, index) => (
              <View key={index} className={`mb-4 ${item.senderModel === 'Tutor' ? 'items-end' : 'items-start'}`}>
                <View className={`p-3 rounded-lg ${item.senderModel === 'Tutor' ? 'bg-blue-500 rounded-br-none' : 'bg-gray-200 rounded-bl-none'}`}>
                  <Text className={item.senderModel === 'Tutor' ? 'text-white' : 'text-gray-800'}>{item.content}</Text>
                </View>
          ...
          <View className="flex-row items-center p-4 bg-white border-t border-gray-200">
            ...
            <TouchableOpacity onPress={sendMessage} disabled={sending}>
              <Ionicons name="paper-plane" size={24} color={sending ? '#CCCCCC' : 'dodgerblue'} />
            </TouchableOpacity>
          </View>
        </View>
      </KeyboardAvoidingView>
